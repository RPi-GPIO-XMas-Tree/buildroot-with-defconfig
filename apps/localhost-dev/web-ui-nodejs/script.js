...
// Culori diferite pentru Non-RGB: LED 0 e Portocaliu, LED 1 e Roz aprins
const nonRgbColors = {
    "0": "rgb(255, 165, 0)", 
    "1": "rgb(255, 20, 147)" 
};
...
async function updateRGBLedsLoop() {
    try {
        const response = await fetch(`${API_BASE}/rgb-leds?t=${Date.now()}`, { 
...
async function updateNonRgbLedsLoop() {
    try {
        const response = await fetch(`${API_BASE}/non-rgb-leds?t=${Date.now()}`, { 
            headers: { 'Cache-Control': 'no-cache' } 
        });
        if (response.ok) {
            const data = await response.json();
            Object.keys(data).forEach(id => {
                ...
                const state = data[id]; // "on", "blinking-on", "blinking-off", "off"

                if (led) {
                    // Logica de iluminare: aprins daca e "on" sau "blinking-on"
                ...
                if (ctrl) {
                    // Sincronizare dropdown: daca e oricare varianta de blinking, selectam "blinking"
...
async function updateCurrentAnimationText() {
    try {
        const response = await fetch(`${API_BASE}/current-animation?t=${Date.now()}`);
        const data = await response.json();
